<script lang="ts">
    ...
    } from "@helper/universalFunctions";
    import Avatar from "@/components/Avatar.svelte";
    import type { Person, TeamManagersObject } from "@/lib/types";

    export let manager: Person,
        leagueTeamManagers: TeamManagersObject,
        key: number;

    let retired = false;

    let year: number | undefined = undefined;
    let rosterID: string | undefined = undefined;

    if (manager.managerID) {
        const dates = getDatesActive(leagueTeamManagers, manager.managerID);
        if (dates && dates.end) retired = true;
...
<a
    ...
    style={retired
        ...
        : ""}
    href={`/manager/${manager.slug?.current}`}
>
    <div class="avatarHolder">
        <Avatar image={manager.image} />
        {#if commissioner}
    ...
    <div class="info">
        ...
        <div class="infoSlot infoRebuild">
            {#if manager.fantasyMode}
                <div class="infoIcon">
                    <img
                        class="infoImg"
                        src="/{manager.fantasyMode.replace(' ', '%20')}.png"
